...
def main():
    ...
    st.title("📽 AI-Powered Subtitle Generator")
    st.markdown("**Automatically generate and search subtitles for your videos and audio files.**")
    
    with st.sidebar:
        st.header("Upload & Search")
        uploaded_file = st.file_uploader("📤 Upload a Video or Audio File", type=["mp4", "mkv", "mp3", "wav"])
        query = st.text_input("🔍 Search for Subtitles", placeholder="Type a keyword...")
        download_btn = st.button("⬇️ Download Subtitles")
    ...
    if uploaded_file:
        ...
        if audio_path:
            ...
            with st.spinner("🔍 Embedding and storing subtitles..."):
                ...
                if embeddings.empty:
                    st.warning("⚠️ No subtitles generated.")
                else:
